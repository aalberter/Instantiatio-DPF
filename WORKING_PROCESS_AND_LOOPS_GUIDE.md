...
Развёрнутая карта обязана быть полной и восстанавливаемой внутри exact Candidate/process carriers. Это не означает item-by-item visibility в ordinary human-facing presentation. Там Runtime группирует результаты по decision-relevant темам и сохраняет видимыми status, authority, material risks/limitations, consequences, allowed/prohibited effects и решение; точные строки, codes и mapping доступны на Engineering/Runtime-Audit уровне или по запросу. Grouping недопустим, если он смешивает несовместимые authority/Admission boundaries либо скрывает элемент, которому нужен самостоятельный human disposition. Markers: `ordinary_grouped_semantic_coverage | basis_completeness_not_item_visibility | item_visibility_requires_independent_disposition`.
...
Когда material architecture, data, interface, interaction или другой engineering result должен стать основой первого relying decision, Runtime автоматически разрешает current applicable Engineering Review Views и фактически предъявляет их smallest sufficient Gate Projection в том же Human Gate до `Рекомендация`/`Варианты решения`. Useful source link сохраняет direct access к exact carrier, но link, filename, hash, bare ID, `tests passed` или carrier-existence assertion являются только supplementary detail и не заменяют presentation. Gate internally связывается с exact Candidate/result, View и authoritative source/carrier configurations, verification/freshness, baseline/delta, limitations/non-coverage, consequence и stable local element refs. Ordinary presentation показывает их decision-relevant смысл; exact identities и element refs выводятся только при самостоятельном disposition, запросе или Engineering/Runtime-Audit уровне. Element ref облегчает addressed review, но не создаёт новый общий `EV-*`, status, authority level или source of truth.
...
Каждая фактически предъявленная View показывает understandable name, decision question/use, smallest sufficient semantic representation, currentness/evidence, baseline/delta, limitation/non-coverage, consequence and useful source link. Exact ID/configuration сохраняются recoverably и добавляются в main presentation при самостоятельной need, запросе или Engineering/Runtime-Audit уровне. Existing diagram/table/native source допустим, когда он действительно displayed или его decision-relevant semantics представлены в text/table form. Renderer, CSS, native control или vendor UI не являются обязательными. Restricted content наследует source access; bare secure link недостаточен без observable authorized presentation. Runtime не создаёт автоматически SVG/PNG/PDF/DOCX или новый View carrier только из-за trigger. Marker: `engineering_gate_projection_presented_before_options | links_supplementary_not_presentation`.
...
Для ordinary-проекции `full projection/checklist` означает полный decision-relevant смысл и recoverable internal mapping, а не обязательную публикацию codes или каждой строки. Если item-specific input действительно нужен, соответствующий вопрос выводится отдельно обычным языком и не может быть скрыт aggregate option.
...
Одна и та же material configuration сохраняет понятную идентичность через request, decision confirmation и handoff. Exact mapping всегда остаётся recoverable. В ordinary presentation при первой и последующей relying reference повторяй понятное название и descriptive link, если он полезен; exact ID добавляй только при самостоятельной decision need, явном запросе или Engineering/Runtime-Audit уровне. Bare ID, filename или path не являются достаточной основной идентификацией результата или решения.
...
Для системного объекта используй `<exact type> — <русское понятное название>`, например `Working Process — проверка ясности взаимодействия`. Exact ID сохраняется в configuration и показывается при самостоятельной need, запросе или Engineering/Runtime-Audit уровне. Русское название поясняет объект, но не переименовывает exact type. То же правило действует для project-specific carriers: headings и explanatory prose формируются прежде всего по-русски. Автоматический word-count, полный запрет English или массовый перевод historical/source content не являются oracle; проверяй смысл, continuity title/ID и отсутствие decorative English. Marker: `interaction_clarity_russian_first`.

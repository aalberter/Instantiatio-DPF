# Instantiatio DPF (iDPF) 3.6.0 — Engineering Work Runtime · Beta

> Версия iDPF Runtime: `3.6.0`; встроенный статус выпуска: `released`; зрелость продукта: `Beta`; выпущенный предшественник: `3.5.0`; включённый AI SDLC DPF: неизменённый `1.0.1`.
...
Этот README входит в пакет iDPF `3.6.0`. Состав изменений и ограничения описаны в [Release Notes 3.6.0](docs/releases/RELEASE_NOTES_3_6_0.md), а точный состав пакета — в [`PACKAGE_MANIFEST.md`](PACKAGE_MANIFEST.md). Встроенный статус `released` не означает, что iDPF сам выполнил внешнюю публикацию: передача, tag, upload и публикация относятся к отдельному проекту и его authority.
...
| [Runtime 3.6 Operational Scenarios](tests/behavioral/RUNTIME_3_6_OPERATIONAL_SCENARIOS.md) | observable behavioral contract изменений версии 3.6.0 |
...
Текущие версии основных компонентов iDPF 3.6.0:

| Компонент | Версия | Состояние |
|---|---:|---|
| iDPF Runtime | `3.6.0` | released · Beta |
...
История развития и направления дальнейшей проверки описаны в [Evolution Roadmap](docs/KIT_EVOLUTION_ROADMAP.md). Изменения конкретной версии находятся в [Release Notes 3.6.0](docs/releases/RELEASE_NOTES_3_6_0.md), а выпущенный предшественник — в [Release Notes 3.5.0](docs/releases/RELEASE_NOTES_3_5_0.md).
...
Публичная identity текущей release configuration — `Instantiatio DPF 3.6.0 — Engineering Work Runtime · Beta`. Выпущенный предшественник — `Instantiatio DPF 3.5.0 · Beta`. Исторические обозначения `Engineering Work Kit 3.1.0`, DPF assembly `Released 2.2.0` и более ранние local baselines сохраняются только как provenance и не являются текущей версией Runtime.
